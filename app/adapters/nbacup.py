...
def list_seasons() -> list[dict]:
    """У NBA Cup выбор сезона пока отключён — прошлые розыгрыши ESPN отдаёт
    неполно, а таблицы групп прошлых лет мы не строим. Показываем только
    текущий розыгрыш (пустой список = селектора сезона нет)."""
    return []
...
def _disabled_fetch_playoff_for(season_code: str) -> list[dict]:
    """ОТКЛЮЧЕНО: плей-офф NBA Cup пока не показываем (текущий розыгрыш ещё
    не начался, а прошлые ESPN отдаёт неполно). Оставлено на будущее.
